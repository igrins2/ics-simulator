...
@click.command(help=CLASS_NAME + " Power control")
def pwrctrl():
    ...
    while(True):
        print("Please input 'index on/off' or 'end':", end=" ")
...
def motormovedelta(motor, delta):
    if (motor == "UT" or motor == "LT") is not True:
        print("Please select UT or LT.")
        return
    ...
    hk = HK()
    hk.ConnectToComponent(5+motornum)
    hk.MotorMoveDelta(motornum, delta)
    hk.CloseComponent(5+motornum)
...
def CliCommand():
    ...
    cli.add_command(motormovedelta)
